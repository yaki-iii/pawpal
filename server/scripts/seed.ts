import { PrismaClient, PetSpecies, CircleType, MembershipLevel } from '@prisma/client';
  ...
  for (const breed of dogBreeds) {
    for (const category of dogCategories) {
      articles.push({
        ...
        breed,
        category,
        content: `# ${breed}${category}完全指南\n\n## 概述\n本文将详细介绍${breatHealthContent(breed, category)}`,
      });
    }
  }
...
function breatHealthContent(breed: string, category: string): string {
...
async function seed(): Promise<void> {
  ...
  // 4. Create knowledge articles (batch for speed)
  console.log('\n📚 Creating knowledge articles...');
  const articles = generateArticles();
  const result = await prisma.knowledgeArticle.createMany({
    data: articles.map(a => ({
      title: a.title,
      species: a.species,
      breed: a.breed,
  ...
  console.log(`✅ Created ${articleCount} knowledge articles`);

  // 5. Create a demo user with a pet
  ...
  // 6. Create a sample community post
  ...
  if (corgiCircle) {
    ...
    await prisma.circle.update({
      ...
      data: { postCount: { increment: 1 }, memberCount: { increment: 1 } },
    });
    await prisma.circleMember.create({
      data: { circleId: corgiCircle.id, userId: demoUser.id },
    });
  }
