...
describe('AIService', () => {
  ...
  describe('runPipeline - Full pipeline with LLM', () => {
    ...
    it('should build sources array from search results', async () => {
      ...
      expect(createData.sources[2].type).toBe('article');
      expect(createData.sources[2].url).toBe('/knowledge/a1');
    });
  });
  ...
  describe('runPipeline - Fallback when LLM not configured', () => {
    ...
    it('should use fallback summary when LLM is not configured', async () => {
      ...
      const createData = (prisma.aIAssistantSession.create as jest.Mock).mock.calls[0][0].data;
      // Fallback summary should contain post/article titles
      expect(createData.summary).toContain('相关帖子');
      expect(createData.summary).toContain('相关文章');
  ...
  describe('buildContextText', () => {
    it('should build context with question, posts, and articles', () => {
  ...
  describe('buildFallbackSummary', () => {
    it('should build summary with posts and articles', () => {
    ...
    it('should handle only posts (no articles or web)', () => {
      ...
      expect(summary).not.toContain('相关知识文章');
    });
  });
